...
class TestSettingsEmailProvider:
    ...
    def test_default_email_provider(self):
        """Test default email provider setting."""
        from app.core.config import settings

        assert hasattr(settings, "default_email_provider")
        # Should be either 'resend' or 'brevo'
        if settings.default_email_provider:
            assert settings.default_email_provider in ["resend", "brevo"]
