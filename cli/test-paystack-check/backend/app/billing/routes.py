...
async def get_plans(request: Request):
    ...
    plans = await Plan.filter(is_active=True).order_by("price")
    return plans
...
async def get_credits_overview(
    ...
    successful_purchases = await Payment.filter(
        user_id=current_user.id, status=PaymentStatus.SUCCESS
    ).all()
